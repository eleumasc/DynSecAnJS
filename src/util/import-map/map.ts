...
export class ImportMap implements IImportMap {
  ...
  setIntegrity(target: string, integrity: string) {
    ...
    if (
      targetRebased.startsWith("./") &&
      target !== targetRebased.slice(2) &&
      this.integrity[targetRebased.slice(2)]
    )
  ...
  getIntegrity(target: string, integrity: string) {
    ...
    if (this.integrity[targetRebased.slice(2)])
      return this.integrity[targetRebased.slice(2)];
  }
