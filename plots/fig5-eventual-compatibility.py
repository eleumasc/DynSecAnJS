...
percentages = {key: [(value / 3721) * 100 for value in values] for key, values in data.items()}
...
rects1 = ax.bar(ind - width/2, [pair[0] for pair in data.values()], width, label='syntactic')
rects2 = ax.bar(ind + width/2, [pair[1] for pair in data.values()], width, label='eventual')

# Aggiunta delle percentuali sopra ciascuna barra
for rects in [rects1, rects2]:
    for rect in rects:
        height = rect.get_height()
        ax.annotate('{:.0f}%'.format(height/3721*100),
